...
failures="{failures}" skipped="0" tests="{tests}" time="1.272" \
...
def write_result(build_base, package, *, tests, errors=0, failures=0,
    ...
    path.write_text(PYTEST_RESULT.format(
        tests=tests, errors=errors, failures=failures, cases=CASE * tests))
    return path
...
def test_results_in_subdirectories_are_found(workspace):
    ...
    (nested / 'suite.xml').write_text(
        PYTEST_RESULT.format(tests=2, errors=0, failures=0, cases=CASE * 2))
