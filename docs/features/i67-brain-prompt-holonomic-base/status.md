...
1. **The drivetrain is the one body claim with no live source, and the head camera is the one body fact still to arrive.** *(Rewritten after the fix landed — the original framing, that body prose had no live source at all and that PR3–PR7 would add four ungated facts, is now false: the arm count, the grippers, the column and the reach are all read from owners, and only the camera is genuinely new.)* What remains: the drivetrain is pinned by a hand-typed ledger rather than an owner, closing that needs the `robot_description` dependency call reversed (D30 records the price and says PR6 lowers it without performing it); and when the head camera reaches the prompt, it lands ungated unless its author finds it an owner or pins it. Paths: `src/robot_brain/test/test_prompt_drift.py`, `src/robot_description/`.
